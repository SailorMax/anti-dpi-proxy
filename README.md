...
### If it doesn't work:
1. try to change `PROXY_COMMAND` variable to "SOCKS 127.0.0.1:1080" in .env-file and find better arguments for "Bypass DPI" (`BYE_DPI_COMMAND`-variable in .env-file). Details: https://github.com/hufrea/byedpi/blob/main/readme.txt
2. try to change `PROXY_COMMAND` variable to "PROXY 127.0.0.1:8888" in .env-file and find better arguments for "Spoof DPI" (`SPOOF_DPI_COMMAND`-variable in .env-file). Details: https://spoofdpi.xvzc.dev/user-guide/https/
