...
if [ -d "${CFG_DIR}" ]; then
	...
	echo "(!)WARNING: ${CFG_DIR} not found."
	DOMAINS_FOR_DPI_PROXY=""
	DOMAINS_FOR_EXT_PROXY=""
fi

# prepare env-variables
GET_DNS_PROXY_IP() {
	local DNS_PROXY_IP=$(nslookup dns-proxy | awk '/^Server:/ { print $2 }' | head -1)
	[ $? -eq 0 ] && echo "${DNS_PROXY_IP}" || echo ""
}
...
case $1 in
	...
	"dnsproxy")
		...
		;;
	"bye_dpi")
		export DNS_PROXY_IP="$(GET_DNS_PROXY_IP)"
		;;
	"spoofdpi")
		export DNS_PROXY_IP="$(GET_DNS_PROXY_IP)"
		;;
	"proxy-chain")  # ext-proxy
		export DNS_PROXY_IP="$(GET_DNS_PROXY_IP)"
